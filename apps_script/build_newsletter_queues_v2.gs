 ...
 *
 * SETUP: enable the advanced service first.
...
// Leave blank on first run; the script creates the destination and logs its ID.
var DEST_ID = '';
...
function writeStatus(dest, scanned, inWindow, nSpecial, nPitches, elapsed) {
  ...
  sheet.getRange(7, 1, 1, 2).setValues([['Run seconds', elapsed]]);
}
